...
// Requête SQL pour récupérer toutes les tâches validées avec la division depuis la table users
$sql = "SELECT t.*, u.division
        FROM tasks t
        JOIN users u ON t.assigned_to = u.IM
        WHERE t.status = 'validee' 
        AND t.etat IN ('En cours', 'Non demarrée', 'terminée', 'Non terminée') 
...
if ($result->num_rows > 0) {
    ...
    while ($task = $result->fetch_assoc()) {
        ...
        $tasks[] = [
            ...
            'division' => $task['division'], // Division récupérée depuis la table users
            'isExpired' => $isExpired,
...
<body>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="extendTaskModalLabel">Prolonger ou Annuler l'activité</h5>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-danger" id="deleteTask">Annuler la activité</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            eventClick: function (info) {
                if (info.event.extendedProps.isExpired === 'true') {
                    const modal = new bootstrap.Modal(document.getElementById('extendTaskModal'));
                    modal.show();

                    document.getElementById('saveNewEndDate').onclick = function () {
                        const newEndDate = document.getElementById('newEndDate').value;
                        if (newEndDate) {
                            updateTaskEndDate(info.event.id, newEndDate);
                            modal.hide();
                        } else {
        ...
        function updateTaskEndDate(taskId, newEndDate) {
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    ...
                    alert("La date de fin a été prolongée !");
                    location.reload(); // Recharger la page après la mise à jour
                }
            };
            xhr.send('taskId=' + taskId + '&newEndDate=' + newEndDate);
        }
        ...
        function cancelTask(taskId) {
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    const event = calendar.getEventById(taskId);
                    event.remove();
                    alert("La tâche a été annulée et son état mis à 'Non terminée' !");
