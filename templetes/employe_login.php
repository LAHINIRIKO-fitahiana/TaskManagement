...
$error = ''; // Déclare la variable pour stocker l'erreur
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "SELECT * FROM users WHERE IM = ? AND role = 'employé'";
    ...
    if ($result->num_rows > 0) {
        $user = $result->fetch_assoc();  // Extraire un utilisateur de la base de données
        $status = $user['status'];
        $role = $user['role'];
        ...
        if ($status != 'active') {
            ...
            if (password_verify($password, $user['password'])) {
                $_SESSION['employeur'] = $user_im;  // Enregistrer le matricule en session
                $_SESSION['roleUser'] = $role;      // Enregistrer le rôle en session
                header("Location: employe_dashboard.php");  // Rediriger vers le tableau de bord
                exit;
            } else {
            ...
            <div class="swiper mySwiper">
                <div class="swiper-wrapper">
                    <div class="swiper-slide">
                        <img src="../assets/images/emp1.png" alt="">
                        <div class="swiper_text">
                            <p>Gestion des tâches personnelles</p>
                        </div>
                    </div>
                    <div class="swiper-slide">
                        <img src="../assets/images/emp2.png" alt="">
                        <div class="swiper_text">
                            <p>Collaborer avec les équipes</p>
                        </div>
                    </div>
                </div>
