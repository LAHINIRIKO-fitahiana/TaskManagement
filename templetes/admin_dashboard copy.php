...
<body>
    ...
    <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdown">
            ...
            <a class="dropdown-item" onclick="window.location.href='?page=valid_compte'">
                <i class="fas fa-user-clock mr-2"></i> Compte en attente de validation: <span><?php echo $pendingAccountsCount; ?></span>
            </a>
            <a class="dropdown-item" onclick="window.location.href='?page=taches_faire'">
                <i class="fas fa-check-circle mr-2"></i> Nouveau activité(s) validée: <span><?php echo $valideeAccountsCount; ?></span>
            </a>
            <a class="dropdown-item" onclick="window.location.href='?page=etat_terminée'">
                <i class="fas fa-check-circle mr-2"></i> Nouveau activité(s) terminé: <span><?php echo $finAccountsCount; ?></span>
            </a>
        </div>
    </li>
</ul>
...
<script>
    document.getElementById('navbarDropdown').addEventListener('click', function () {
        // Envoyer une requête AJAX pour marquer les notifications comme vues
        fetch('mark_all_notifications_seen.php')
            ...
            .catch(error => console.error('Error:', error));
                ...
                <div class="dropdown ml-3">
                    <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownUser" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" title="Mon Compte">
                        <i class="fas fa-user"></i>
                    </button>
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownUser">
                        <div class="dropdown-divider"></div>
                        <a href="#" class="dropdown-item" data-toggle="modal" data-target="#createAdminModal">
                            <i class="fas fa-user-plus me-2"></i>&nbsp;&nbsp;Ajouter un autre admin
                        </a>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (empty($im) || empty($username) || empty($email) || empty($password)) {
...
document.getElementById('navbarDropdown').addEventListener('click', function (e) {
    ...
    // Lorsqu'on clique sur le bouton "Mon Compte"
    document.getElementById('dropdownUser').addEventListener('click', function (e) {
        e.preventDefault();  // Empêche le rechargement de la page
        
        var dropdownMenu = this.nextElementSibling;  // Trouve le menu déroulant
        var isOpen = dropdownMenu.classList.contains('show');  // Vérifie si le menu est ouvert
    ...
    document.addEventListener('DOMContentLoaded', function () {
        // Sélectionner le bouton ou lien qui ouvre le modal
        const openModalButton = document.getElementById('openCreateAdminModal');

        // Sélectionner le modal Bootstrap
        const createAdminModal = new bootstrap.Modal(document.getElementById('createAdminModal'));

        // Ajouter un événement pour afficher le modal
        openModalButton.addEventListener('click', function () {
            createAdminModal.show(); // Utiliser createAdminModal.show() pour afficher la modal
        });
        ...
        document.getElementById('createAdminModal').addEventListener('hidden.bs.modal', function () {
            location.reload(); // Recharger la page quand la modal se ferme
        });
    });
